...
% it to obs.upd/dcb_L1/L2/L3/C1/C2/C3 for RINEX 2
% INPUT:
%   obs     struct, ...
%   input   struct, ...
% OUTPUT:
...
if bool_P1
    obs.C1_corr(:,1:32) = input.ORBCLK.corr2brdc_GPS.dcb.C1W;
    obs.used_biases_GPS{2,1} = 'C1W';
else
    obs.C1_corr(:,1:32) = input.ORBCLK.corr2brdc_GPS.dcb.C1C;
    obs.used_biases_GPS{2,1} = 'C1C';
end
idx_c2 = obs.use_column{1, 5};
bool_P2 = strcmp('P2', obs.types_gps(2*idx_c2-1:2*idx_c2));
if bool_P2
    obs.C2_corr(:,1:32) = input.ORBCLK.corr2brdc_GPS.dcb.C2W;
    obs.used_biases_GPS{2,2} = 'C2W';
else
    obs.C2_corr(:,1:32) = input.ORBCLK.corr2brdc_GPS.dcb.C2C;
    obs.used_biases_GPS{2,2} = 'C2C';
end

% Phase Biases
if bool_P1
    obs.L1_corr(:,1:32) = input.ORBCLK.corr2brdc_GPS.upd.L1W;
    obs.used_biases_GPS{1,1} = 'L1W';
else
    obs.L1_corr(:,1:32) = input.ORBCLK.corr2brdc_GPS.upd.L1C;
    obs.used_biases_GPS{1,1} = 'L1C';
end
obs.L2_corr(:,1:32) = input.ORBCLK.corr2brdc_GPS.upd.L2W;
obs.used_biases_GPS{2,1} = 'L2W';

% set to zero because 3rd frequency does not exist in Rinex 2:
obs.C3_corr = 0*input.ORBCLK.corr2brdc_GPS.dcb.C2W;
obs.L3_corr = 0*input.ORBCLK.corr2brdc_GPS.upd.L2W;
