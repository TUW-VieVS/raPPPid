...
% it to obs.upd/dcb_L1/L2/L3/C1/C2/C3
%
% INPUT:
...
if settings.BIASES.code_corr2brdc_bool
    ...
    if settings.INPUT.use_GPS
        code_gps = fieldnames(input.ORBCLK.corr2brdc_GPS.dcb);             % fieldnames of GPS code bias corrections
        if isfield(obs.GPS, 'C1') && ~isempty(obs.GPS.C1)
            [obs.C1_corr(:,gps_cols), obs.used_biases_GPS{1,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GPS.dcb, obs.C1_corr(:,gps_cols), code_gps, obs.GPS.C1, 'GPS C1');
        end
        if isfield(obs.GPS, 'C2') && ~isempty(obs.GPS.C2)
            [obs.C2_corr(:,gps_cols), obs.used_biases_GPS{2,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GPS.dcb, obs.C2_corr(:,gps_cols), code_gps, obs.GPS.C2, 'GPS C2');
        end
        if isfield(obs.GPS, 'C3') && ~isempty(obs.GPS.C3)
            [obs.C3_corr(:,gps_cols), obs.used_biases_GPS{3,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GPS.dcb, obs.C3_corr(:,gps_cols), code_gps, obs.GPS.C3, 'GPS C3');
        end
    end
    
    % -- Glonass
    if settings.INPUT.use_GLO
        code_glo = fieldnames(input.ORBCLK.corr2brdc_GLO.dcb);             % fieldnames of Glonass code bias corrections
        if isfield(obs.GLO, 'C1') && ~isempty(obs.GLO.C1)
            [obs.C1_corr(:,glo_cols), obs.used_biases_GLO{1,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GLO.dcb, obs.C1_corr(:,glo_cols), code_glo, obs.GLO.C1, 'Glonass C1');
        end
        if isfield(obs.GLO, 'C2') && ~isempty(obs.GLO.C2)
            [obs.C2_corr(:,glo_cols), obs.used_biases_GLO{2,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GLO.dcb, obs.C2_corr(:,glo_cols), code_glo, obs.GLO.C2, 'Glonass C2');
        end
        if isfield(obs.GLO, 'C3') && ~isempty(obs.GLO.C3)
            [obs.C3_corr(:,glo_cols), obs.used_biases_glo{3,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GLO.dcb, obs.C3_corr(:,glo_cols), code_glo, obs.GLO.C3, 'Glonass C3');
        end
    end
    
    % -- Galileo --
    if settings.INPUT.use_GAL
        code_gal = fieldnames(input.ORBCLK.corr2brdc_GAL.dcb);             % fieldnames of Galileo code bias corrections
        if isfield(obs.GAL, 'C1') && ~isempty(obs.GAL.C1)
            [obs.C1_corr(:,gal_cols), obs.used_biases_GAL{1,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GAL.dcb, obs.C1_corr(:,gal_cols), code_gal, obs.GAL.C1, 'Galileo C1');
        end
        if isfield(obs.GAL, 'C2') && ~isempty(obs.GAL.C2)
            [obs.C2_corr(:,gal_cols), obs.used_biases_GAL{2,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GAL.dcb, obs.C2_corr(:,gal_cols), code_gal, obs.GAL.C2, 'Galileo C2');
        end
        if isfield(obs.GAL, 'C3') && ~isempty(obs.GAL.C3)
            [obs.C3_corr(:,gal_cols), obs.used_biases_GAL{3,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GAL.dcb, obs.C2_corr(:,gal_cols), code_gal, obs.GAL.C3, 'Galileo C3');
        end
    end
    
    % -- BeiDou --
    if settings.INPUT.use_BDS
        code_bds = fieldnames(input.ORBCLK.corr2brdc_BDS.dcb);             % fieldnames of BeiDou code bias corrections
        if isfield(obs.BDS, 'C1') && ~isempty(obs.BDS.C1)
            [obs.C1_corr(:,bds_cols), obs.used_biases_BDS{1,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_BDS.dcb, obs.C1_corr(:,bds_cols), code_bds, obs.BDS.C1, 'BeiDou C1');
        end
        if isfield(obs.BDS, 'C2') && ~isempty(obs.BDS.C2)
            [obs.C2_corr(:,bds_cols), obs.used_biases_BDS{2,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_BDS.dcb, obs.C2_corr(:,bds_cols), code_bds, obs.BDS.C2, 'BeiDou C2');
        end
        if isfield(obs.BDS, 'C3') && ~isempty(obs.BDS.C3)
            [obs.C3_corr(:,bds_cols), obs.used_biases_BDS{3,2}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_BDS.dcb, obs.C2_corr(:,bds_cols), code_bds, obs.BDS.C3, 'BeiDou C3');
        end
    end
end
...
if settings.BIASES.phase_corr2brdc_bool
    ...
    % -- GPS --
    if settings.INPUT.use_GPS
        gps_phase = fieldnames(input.ORBCLK.corr2brdc_GPS.upd);         % fieldnames of GPS dcb corrections
        if isfield(obs.GPS, 'L1') && ~isempty(obs.GPS.L1)
            [obs.L1_corr(:,gps_cols), obs.used_biases_GPS{1,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GPS.upd, obs.L1_corr(:,gps_cols), gps_phase, obs.GPS.L1, 'GPS L1');
        end
        if isfield(obs.GPS, 'L2') && ~isempty(obs.GPS.L2)
            [obs.L2_corr(:,gps_cols), obs.used_biases_GPS{2,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GPS.upd, obs.L2_corr(:,gps_cols), gps_phase, obs.GPS.L2, 'GPS L2');
        end
        if isfield(obs.GPS, 'L3') && ~isempty(obs.GPS.L3)
            [obs.L3_corr(:,gps_cols), obs.used_biases_GPS{3,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GPS.upd, obs.L3_corr(:,gps_cols), gps_phase, obs.GPS.L3, 'GPS L3');
        end
    end
    ...
    % -- Galileo --
    if settings.INPUT.use_GAL
        gal_phase = fieldnames(input.ORBCLK.corr2brdc_GAL.upd);         % fieldnames of GPS dcb corrections
        if isfield(obs.GAL, 'L1') && ~isempty(obs.GAL.L1)
            [obs.L1_corr(:,gal_cols), obs.used_biases_GAL{1,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GAL.upd, obs.L1_corr(:,gal_cols), gal_phase, obs.GAL.L1, 'Galileo L1');
        end
        if isfield(obs.GAL, 'L2') && ~isempty(obs.GAL.L2)
            [obs.L2_corr(:,gal_cols), obs.used_biases_GAL{2,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GAL.upd, obs.L2_corr(:,gal_cols), gal_phase, obs.GAL.L2, 'Galileo L2');
        end
        if isfield(obs.GAL, 'L3') && ~isempty(obs.GAL.L3)
            [obs.L3_corr(:,gal_cols), obs.used_biases_GAL{3,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_GAL.upd, obs.L3_corr(:,gal_cols), gal_phase, obs.GAL.L3, 'Galileo L3');
        end
    end
    
    % -- BeiDou --
    if settings.INPUT.use_BDS
        bds_phase = fieldnames(input.ORBCLK.corr2brdc_BDS.upd);         % fieldnames of GPS dcb corrections
        if isfield(obs.BDS, 'L1') && ~isempty(obs.BDS.L1)
            [obs.L1_corr(:,bds_cols), obs.used_biases_BDS{1,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_BDS.upd, obs.L1_corr(:,bds_cols), bds_phase, obs.BDS.L1, 'BeiDou L1');
        end
        if isfield(obs.BDS, 'L2') && ~isempty(obs.BDS.L2)
            [obs.L2_corr(:,bds_cols), obs.used_biases_BDS{2,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_BDS.upd, obs.L2_corr(:,bds_cols), bds_phase, obs.BDS.L2, 'BeiDou L2');
        end
        if isfield(obs.BDS, 'L3') && ~isempty(obs.BDS.L3)
            [obs.L3_corr(:,bds_cols), obs.used_biases_BDS{3,1}] = ...
                find_corr2brdc_bias(input.ORBCLK.corr2brdc_BDS.upd, obs.L3_corr(:,bds_cols), bds_phase, obs.BDS.L3, 'BeiDou L3');
        end
    end
end
