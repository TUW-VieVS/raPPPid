function vis_plotSatConstellation(hours, epochs, strXAxis, satellites, cutoff, isGPS, isGLO, isGAL, isBDS, isQZS)
...
if isQZS && any(noAllQZS)
    area(hours, isGPS*noAllGPS+isGLO*noAllGLO+isGAL*noAllGAL+noAllBDS+noAllQZS, 'FaceColor', DEF.COLOR_J, 'EdgeColor','none')
    legend_txt{end+1} = 'QZSS';
end
if isBDS && any(noAllBDS)
    area(hours, isGPS*noAllGPS+isGLO*noAllGLO+isGAL*noAllGAL+noAllBDS, 'FaceColor', DEF.COLOR_C, 'EdgeColor','none')
    legend_txt{end+1} = 'BeiDou';
end
if isGAL && any(noAllGAL)
    area(hours, isGPS*noAllGPS+isGLO*noAllGLO+noAllGAL, 'FaceColor', DEF.COLOR_E, 'EdgeColor','none')
    legend_txt{end+1} = 'Galileo';
end
if isGLO && any(noAllGLO)
    area(hours, isGPS*noAllGPS+noAllGLO, 'FaceColor', DEF.COLOR_R, 'EdgeColor','none')
    legend_txt{end+1} = 'GLONASS';
end
if isGPS && any(noAllGPS)
    area(hours, noAllGPS, 'FaceColor', DEF.COLOR_G, 'EdgeColor','none')
...
if isGPS
    ...
    plot(hours, UsedGPS,  '.', 'Color',DEF.COLOR_G)
...
if isQZS
    plot(hours, sv_QZS-400.3, '.m');
    plot(hours, sv_QZS_cutoff+0.08, '.', 'Color',[0.8 0.8 0.8]);
end
Grid_Xoff_Yon()
yticks(0:32)
xlabel(strXAxis)
