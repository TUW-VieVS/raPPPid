...
handles.zwd_est   = param(:,idx_zwd);    % estimated ZWD
handles.IGS_est   = []; handles.checkbox_IGS_ZTD.Enable = 'off';
handles.leg_txt   = [];
handles.ff = ff;

% check if IGS troposphere file already exists
if ~isempty(handles.station)
    STAT = upper(handles.station);
    stat = lower(handles.station(1:4));
    ...
    if (yyyy > 2023) || (yyyy == 2023 && doy >= 48)
        ...
        % ||| not working
        tropofile = [Path.DATA 'TROPO/' yyyy_ '/' doy_ '/' 'IGS0OPSFIN_' yyyy_ doy_ '0000_01D_05M_' STAT '_TRO.TRO'];
    else
        tropofile = [Path.DATA 'TROPO/' yyyy_ '/' doy_ '/' stat doy_ '0.' yyyy_(3:4) 'zpd'];
    end
...
% style
title(['Troposphere Plot, ' handles.ff])
xlabel(label_x)
ylabel('Delay [m]')
grid on;
...
xlim(4*[-std_trop-bias_trop std_trop+bias_trop])
ylabel('[%]')
yticklabels(yticks*100)
legend(str_legend(1:end-3))
