...
%
% INPUT:
%   Value       epochs x 399, SNR or residuals of all satellites and epochs, matrix
...
%   LUT         Look-Up-Table, colors for colorcoding of SNR
%
...
if bool_def
    ...
    val_min = floor(min(Value(:)));       % lowest SNR-value of all satellites
    val_max = floor(max(Value(:))+1);     % highest SNR-value of all satellites
    if isnan(val_min)
        val_min = 0;
    end
    if isnan(val_max)
