function CodeDifference(x, storeData, label_x, resets, rgb, settings, satellites)
...
thresh = settings.OTHER.mp_thresh;
ms = 9;         % Marker Size
fig_mp = figure('Name', ['Code difference, degree: ' num2str(settings.OTHER.mp_degree)], 'NumberTitle','off');
...
if settings.INPUT.use_GPS
    ...
    plot_code_difference(n_plot, x, C1_diff, obs_prns_G, prns_string_G, label_x, thresh, ms, i_plot)
    title('GPS')
    i_plot = i_plot + 1;
end
if settings.INPUT.use_GLO
    ...
    plot_code_difference(n_plot, x, C1_diff, obs_prns_R, prns_string_R, label_x, thresh, ms, i_plot)
    title('GLONASS')
    i_plot = i_plot + 1;
end
if settings.INPUT.use_GAL
    ...
    plot_code_difference(n_plot, x, C1_diff, obs_prns_E, prns_string_E, label_x, thresh, ms, i_plot)
    title('Galileo')
    i_plot = i_plot + 1;
end
if settings.INPUT.use_BDS
    ...
    plot_code_difference(n_plot, x, C1_diff, obs_prns_C, prns_string_C, label_x, thresh, ms, i_plot)
    title('BeiDou')
end
...
function [] = plot_code_difference(n_plot, x, C1_diff, obs_prns_gnss, prns_string_gnss, label_x, thresh, ms, i_plot)
