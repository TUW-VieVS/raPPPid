...
%% Preparations
time = Epoch.gps_time;          % time of current epoch in [sow]
...
if settings.ORBCLK.bool_precise && (~contains(settings.IONO.model, 'IF-LC') || contains(settings.BIASES.code, 'Multi-GNSS DCBs'))
...
elseif settings.ORBCLK.bool_brdc && strcmp(settings.ORBCLK.CorrectionStream,'CNES Archive') 
    % CNES Biases are applied directly on the raw observation
    k_vec_1 = [];     k_vec_2 = [];
end
...
% --- Frequency 1
Epoch.C1_bias = debias(k_vec_2, obs.C1_bias, bool_sats, time, Epoch.sats);
Epoch.L1_bias = debias(k_vec_2, obs.L1_bias, bool_sats, time, Epoch.sats);

% --- Frequency 2
if settings.INPUT.num_freqs > 1
    Epoch.C2_bias = debias(k_vec_2, obs.C2_bias, bool_sats, time, Epoch.sats);
    Epoch.L2_bias = debias(k_vec_2, obs.L2_bias, bool_sats, time, Epoch.sats);
end

% --- Frequency 3
if settings.INPUT.num_freqs > 2
    Epoch.C3_bias = debias(k_vec_2, obs.C3_bias, bool_sats, time, Epoch.sats);
    Epoch.L3_bias = debias(k_vec_2, obs.L3_bias, bool_sats, time, Epoch.sats);
end
...
function bias_epoch = debias(k_vec, BIAS, bool_sats, time, sats)
...
%   time            current point in time, [sow]
...
% find valid biases
valid_start = start(:,bool_sats,:) <= time;   	% find indices of biases which are before current point in time
valid_ende  = ende(:,bool_sats,:)  >  time;   	% find indices of biases which are after current point in time
take = valid_start & valid_ende;
