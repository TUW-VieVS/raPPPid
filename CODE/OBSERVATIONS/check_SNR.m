...
% SNR cutoff
if ~isnan(settings.PROC.SNR_mask)
    ...
    % check for exclusion
    remove_SNR = SNR < settings.PROC.SNR_mask;
    remove_SNR = any(remove_SNR, 2);        % if one SNR observation is under threshold, whole satellite is excluded
    % removed satellites: set number of tracked epochs to zero
    Epoch.tracked(Epoch.sats(remove_SNR)) = 0;
    % exclude
    Epoch.exclude(remove_SNR,:) = true;
