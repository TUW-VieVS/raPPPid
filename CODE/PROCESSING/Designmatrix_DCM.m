...
n3 = n * (num_freq >= 3);       % number of satellites (third frequency)
...
f2nd = [zeros(n,1); ones(n,1);  zeros(n3,1)];
f3rd = [zeros(n,1); zeros(n,1); ones(n3,1)];
...
A(code_row,:)  = [xyz, T, rx_clk_c, IFB,  O_n5,    O_n5,    O_nn ] .*  ~exclude;
A(phase_row,:) = [xyz, T, rx_clk_p, O_n5, bias_L2, bias_L3, amb_p] .*  ~exclude .* usePhase;
