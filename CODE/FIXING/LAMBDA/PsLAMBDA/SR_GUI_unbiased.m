function SR_GUI_unbiased(action, option1);
...
switch action;
    ...
    case 'initialize'
        ...
        h = uicontrol ( 'Style','PushButton','Units','normalized',...
            ...
            'String', 'browse', 'Callback','SR_GUI_unbiased Selectfile');
...
%        
        ...
        h = uicontrol ('Style','RadioButton','Units','normalized',...
            ...
            'Callback',['SR_GUI_unbiased select ILS'],'Tag','ILSall', 'Value',0,'Enable','On');
        ...
        textBottom = textBottom - (editHeight+editSpacing);
        h = uicontrol('Style','CheckBox', 'Units','normalized', 'Position',[textLeft textBottom 1.3*editWidth editHeight], 'String','EXACT', 'BackgroundColor',textBackgroundColor, 'Tag','boot1', 'Value', 1, 'Callback', ['SR_GUI_unbiased boot 1'], 'Enable','On');
        textBottom = textBottom - (editHeight+editSpacing);
        h = uicontrol('Style','CheckBox', 'Units','normalized', 'Position',[textLeft textBottom 1.3*editWidth editHeight], 'String','UB : ADOP-based', 'BackgroundColor',textBackgroundColor, 'Tag','boot2', 'Value',0, 'Callback', ['SR_GUI_unbiased boot 2'],'Enable','On');
        textBottom = textBottom - (editHeight+editSpacing);
        h = uicontrol ('Style','RadioButton','Units','normalized',...
            'Position',[textLeft textBottom 1.3*editWidth editHeight],...
            'String','(de)select all', 'BackgroundColor',textBackgroundColor, ...
            'Callback',['SR_GUI_unbiased select B'],'Tag','Ball', 'Value',0,'Enable','On');
        ...
        textBottom = textBottom - (editHeight+editSpacing);
        h = uicontrol('Style','CheckBox', 'Units','normalized', 'Position',[textLeft textBottom 1.3*editWidth editHeight], 'String','AP : simulation-based', 'BackgroundColor',textBackgroundColor, 'Tag','round1', 'Value',0, 'Callback', ['SR_GUI_unbiased round 1'], 'Enable','On');

        textBottom = textBottom - (editHeight+editSpacing);
        h = uicontrol('Style','CheckBox', 'Units','normalized', 'Position',[textLeft textBottom 1.3*editWidth editHeight], 'String','LB', 'BackgroundColor',textBackgroundColor, 'Tag','round2', 'Value',0, 'Callback', ['SR_GUI_unbiased round 2'], 'Enable','On');
        textBottom = textBottom - (editHeight+editSpacing);
        h = uicontrol('Style','CheckBox', 'Units','normalized', 'Position',[textLeft textBottom 1.3*editWidth editHeight], 'String','UB', 'BackgroundColor',textBackgroundColor, 'Tag','round3', 'Value',1, 'Callback', ['SR_GUI_unbiased round 3'], 'Enable','On');
        textBottom = textBottom - (editHeight+editSpacing);
        h = uicontrol ('Style','RadioButton','Units','normalized',...
            'Position',[textLeft textBottom editWidth editHeight],...
            'String','(de)select all', 'BackgroundColor',textBackgroundColor, ...
            'Callback',['SR_GUI_unbiased select R'],'Tag','Rall', 'Value',0,'Enable','On');
        ...
        frmHeight      = frmPosition(2) - 11*frmSpacing;
        frmPosition(1) = frmSpacing;
        frmPosition(2) = 9*frmSpacing;
        frmPosition(3) = frmWidth;
        frmPosition(4) = frmHeight;
        % --- compute ---
        btnLeft     = frmSpacing + (frmWidth-btnWidth)/2;
        btnBottom   = 8*frmSpacing + frmHeight -btnHeight-btnSpacing;
        btnPosition = [btnLeft btnBottom btnWidth btnHeight];
        compHndl = uicontrol ( 'Style','Push','Units','normalized','Backgroundcolor',btnColor, 'Position',btnPosition, 'String','COMPUTE', 'Callback','SR_GUI_unbiased ''compute''');
        % --- Info button ---
        btnBottom   = btnBottom - btnHeight - btnSpacing;
        btnPosition = [btnLeft btnBottom btnWidth btnHeight];
        InfoHndl = uicontrol ( 'Style','Push', 'Units','normalized', 'Position',btnPosition,'Backgroundcolor',btnColor, 'String','HELP','Callback','SR_GUI_unbiased ''about''');
        % --- defaults button ---
        btnBottom   = btnBottom - btnHeight - btnSpacing;
        btnPosition = [btnLeft btnBottom btnWidth btnHeight];
        defaHndl = uicontrol ('Style','Push', 'Units','normalized', 'Position',btnPosition,'String','DEFAULTS','Backgroundcolor',btnColor,'Callback','SR_GUI_unbiased ''default''');
        % --- close ---
        btnBottom   = btnBottom - btnHeight - btnSpacing;
        btnPosition = [btnLeft btnBottom btnWidth btnHeight];
    ...
    case 'compute'
        ...
        Qa = Q;  clear Q
                ...
                if get (findobj (gcf,'Tag','ILS1'),'Value'); mopt = [mopt 1]; ii=ii+1; string(ii,1:33)=' '; string(ii,1:20)='AP: simulation-based'; end;
                if get (findobj (gcf,'Tag','ILS2'),'Value'); mopt = [mopt 2]; ii=ii+1; string(ii,1:33)=' '; string(ii,1:14)='AP: ADOP-based';      end;
                if get (findobj (gcf,'Tag','ILS3'),'Value'); mopt = [mopt 3]; ii=ii+1; string(ii,1:33)=' '; string(ii,1:17)='LB: bootstrapping';    end;
                if get (findobj (gcf,'Tag','ILS4'),'Value'); mopt = [mopt 4]; ii=ii+1; string(ii,1:33)=' '; string(ii,1:19)='LB: bounding region';  end;
                if get (findobj (gcf,'Tag','ILS5'),'Value'); mopt = [mopt 5]; ii=ii+1; string(ii,1:33)=' '; string(ii,1:22)='LB: bounding vc-matrix';    end;
                if get (findobj (gcf,'Tag','ILS7'),'Value'); mopt = [mopt 6]; ii=ii+1; string(ii,1:33)=' '; string(ii,1:14)='UB: ADOP-based';          end;
                if get (findobj (gcf,'Tag','ILS8'),'Value'); mopt = [mopt 7]; ii=ii+1; string(ii,1:33)=' '; string(ii,1:19)='UB: bounding region';  end;
                if get (findobj (gcf,'Tag','ILS9'),'Value'); mopt = [mopt 8]; ii=ii+1; string(ii,1:33)=' '; string(ii,1:22)='UB: bounding vc-matrix';     end;
                ...
                method = 2;
                mopt = [];  mopt2 = []; Ps = [];  ii = 0;
                if get (findobj (gcf,'Tag','boot1'),'Value'); mopt = [mopt 1]; ii=ii+1; stringB(ii,1:33)=' '; stringB(ii,1:18)='Exact success rate'; end;
                if get (findobj (gcf,'Tag','boot2'),'Value'); mopt = [mopt 2]; ii=ii+1; stringB(ii,1:33)=' '; stringB(ii,1:14)='UB: ADOP-based';      end;
                ...
                method = 3;
                mopt = []; mopt2 = []; Ps = [];  ii = 0;
                if get (findobj (gcf,'Tag','round1'),'Value'); mopt = [mopt 1]; ii=ii+1; stringR(ii,1:33)=' '; stringR(ii,1:20)='AP: simulation-based'; end;
                if get (findobj (gcf,'Tag','round2'),'Value'); mopt = [mopt 2]; ii=ii+1; stringR(ii,1:33)=' '; stringR(ii,1:2)='LB'; end;
                if get (findobj (gcf,'Tag','round3'),'Value'); mopt = [mopt 3]; ii=ii+1; stringR(ii,1:33)=' '; stringR(ii,1:17)='UB: bootstrapping'; end;
